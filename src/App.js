...
import './App.css';
import { getSongList, getSongListbyQuery} from './api';

function App() {
  const [songList, setSongList] = useState([])
  useEffect(() => {
    getSongList().then((result) => {
      setSongList(result)
    })
  }, [])
    ...
    return songList.map((song, i) => {
      return (
        <div className="max-w-sm rounded overflow-hidden shadow-lg bg-gray-800 border border-gray-700" key={i}>
          ...
          <div className="font-bold text-white text-lg truncate mb-2 h-10 py-0">{song.result.full_title}</div>
            <img src={song.result.song_art_image_thumbnail_url} alt="" />
            <div className="text-white text-xl mb-2 italic text-center">{song.result.release_date_for_display}</div>
            <div className="text-white text-xl mb-2">{song.vote_average}</div>
          </div>
        </div>
      );
    });
  ...
  async function search(q) {
    if (q.length >= 3) {
      const query = await getSongListbyQuery(q);
      setSongList(query);
    }
  }
